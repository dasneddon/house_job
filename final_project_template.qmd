...
@bartik1991 sought to isolate exogenous national trends for analysis on the effect of local policies. This paper seeks to do the same for local shifts in the labor market, specifically the "mix" of the labor market by industry for analysis of median rents in each Metropolitan Statistical Area (MSA) in the United States.
...
emplhousing <- emplhousing %>%
  mutate(OCC_GROUP = case_when(
    ...
    OCC_TITLE %in% "Farming, Fishing, and Forestry Occupations" ~ "Agriculture",
    
    OCC_TITLE %in% "Arts, Design, Entertainment, Sports, and Media Occupations" ~ "Entertainment",
...
```{r}
nsa_index <- data.frame("AREAYEAR"= emplhousing$AREAYEAR, "index_nsa" = emplhousing$index_nsa)
nsa_index <- nsa_index[!duplicated(nsa_index),]
emplhousing_AO <- emplhousing[emplhousing$O_GROUP == "total",]
emplhousing_AO <- emplhousing_AO[,c("AREAYEAR", "YEAR", "TOT_EMP")]
emplhousing_AO <- emplhousing_AO %>%
  rename(AO_EMP = TOT_EMP)
empl_ANNUAL <- aggregate(AO_EMP ~ YEAR, data = emplhousing_AO, FUN = sum)
emplhousing_AO <- emplhousing_AO[,c("AREAYEAR", "AO_EMP")]
emplhousing <- emplhousing[emplhousing$O_GROUP != "total",]
emplhousing_group <- aggregate(TOT_EMP ~ OCC_GROUP + AREA + AREAYEAR + AREA_TITLE + YEAR, data = emplhousing, FUN = sum)
emplhousing_group <- merge(emplhousing_group, emplhousing_AO, by = "AREAYEAR")
emplhousing_group <- merge(emplhousing_group, demos, by = "AREA" )
...
industry_shares <- emplhousing_group[emplhousing_group$YEAR == 2016,]
industry_shares$nat_share <- industry_shares$TOT_EMP/industry_shares$AO_EMP
industry_shares <- industry_shares[,c("AREA", "OCC_GROUP", "nat_share")]
...
```{r}
natl_shocks <- aggregate(TOT_EMP ~ OCC_GROUP + YEAR, data = emplhousing_group, FUN = sum)

library(plm)
natl_shocks <- pdata.frame(natl_shocks, index = c("OCC_GROUP", "YEAR"))
natl_shocks$nat_growth <- (natl_shocks$TOT_EMP - plm::lag(natl_shocks$TOT_EMP, 1)) / plm::lag(natl_shocks$TOT_EMP, 1) 
```

```{r}

nat_emp <- natl_shocks[, c("OCC_GROUP", "YEAR", "nat_growth")]
emplhousing_group <- merge(emplhousing_group, nat_emp, by=c("OCC_GROUP", "YEAR"))
ay_growth <- pdata.frame(aggregate(TOT_EMP ~ AREA + YEAR + AREAYEAR, data = emplhousing, FUN = sum), index = c("AREA","YEAR"))
ay_growth$true_growth <- (ay_growth$TOT_EMP - plm::lag(ay_growth$TOT_EMP, 1)) / plm::lag(ay_growth$TOT_EMP,1)
ay_growth <- ay_growth[,c("AREAYEAR", "true_growth")]
emplhousing_group <- merge(emplhousing_group, industry_shares, by=c("AREA", "OCC_GROUP"))
emplhousing_group$loc_share <- emplhousing_group$TOT_EMP/emplhousing_group$AO_EMP
emplhousing_group$natl_shock_wt <- emplhousing_group$nat_share*emplhousing_group$nat_growth
emplhousing_group <- merge(emplhousing_group, ay_growth, by = "AREAYEAR" )
emplhousing_group <- merge(emplhousing_group, nsa_index, by = "AREAYEAR")
```
...
emplhousing_group <- emplhousing_group %>%
  mutate(across(
    c("inc2016", "tot_pop", "college_pct", "over25_pct", "true_growth", "natl_shock_wt", "index_nsa"),
    as.numeric
...
emplhousing_group <- na.omit(emplhousing_group)
emplhousing_group <- as.data.frame(emplhousing_group)
iv_model <- feols(index_nsa ~ OCC_GROUP + loc_share| AREA + YEAR | true_growth ~ natl_shock_wt, data = emplhousing_group)
summary(iv_model)
