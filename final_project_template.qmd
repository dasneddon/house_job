---
title: "Applying the Bartik Shift Share Instrument to the Local Housing Market"
...
The objective of this paper is to demonstrate a causal effect of local employment shocks on the price of housing. However, national effects shocks can confound the local effects, to address this confounder I intend to use a bartik instrument to control for the national effects.
...
```
The dataset before calculations are applied is available here -> [@Sneddon2025jobdata_no_inst]
...
Housing shocks, measuring percent change of housing price.
```{r}
...
```
Final data cleanup to remove any missing values. For these purposes, missing values mean any MSA, year, and occupation group combination with missing values.
...
First we can examine a fixed effects regression to examine correlation.

$$
arcsinh(Y_{lt}) = \beta_0\Delta L_{lt} + \beta_{nlt} X_{nlt} + \alpha_l + \theta_t + \varepsilon_{lt}
$${#eq-fereg}
Where:

$Y_{lt}$ is the expected median home value in a given MSA, $l$, in a given year $t$. This regression is using an inverse hyperbolic sine transformation[^1] as I am ultimately looking for an elasticity.\
$\Delta L_{lt}$ is a local shock in a given local industry $L$.\
$\alpha_l$ are individual fixed effects for a given MSA.\
$\theta_t$ are time fixed effects for a given year.

[^1]: It is unlikely that there would be an issue with using $ln(Y_{lt})$ here, however I prefer the practice of using $arcsinh$ to avoid losing data.

```{r}
 fe_model <- feols(asinh(median_home_value) ~ 
...
modelsummary(fe_model, 
             data = job_demo, 
             title = "Regression with MSA and Year Fixed Effects",
             fmt = fmt_significant(3),

             stars = TRUE,
             gof_map = c("nobs", "r.squared"),
             coef_map = c("industry_pool::business_services:loc_growth" = "Business Services × Local Growth",
                          "industry_pool::consumer_services:loc_growth" = "Consumer Services × Local Growth",
                          "industry_pool::goods_producing:loc_growth" = "Goods Producing × Local Growth",
                          "industry_pool::logistics_util:loc_growth" = "Logistics/Utilities × Local Growth",
                          "college_pct" = "% College Degree",
                          "over25_pct" = "% Residents over 25",
                          "asinh(inc)" = "Median Income",
                          "asinh(tot_pop)" = "MSA Population")
)

```
There are some statistically significant findings here, even if we can't consider them causal as the local shocks may be confounded by national shocks. Using public services as a baseline. It appears that growth in any other industry will have a negative correlation on housing prices, with goods production and logistics allowing us to reject $H_0$ at $95\%$ certainty. Notably, median income and MSA population have a statistically significant yet miniscule postitive effect.

In order to make a possible causal inference, we need to address the the confounder of national trends. Using the bartik instrument we can do a two stage regression:

First Stage:

$$
\Delta L_{lt} = \gamma + \tau Z_{lt} + \alpha_l + \theta_t + \varepsilon_{lt} : Z_{lt} = \sum_k g_{kt} \times g_{2010,lk}
$${#eq-firststage}

$\Delta I_{lt}$: This is the local industry variable subject to treatment by the bartik instrument.
$Z_{lt}$: This is the bartik instrument. The sum of all national industry shares over time $g_{kt}$ times the baseline industry share in 2010 in each location $g_{2010,lk}$.

Second Stage:

$$
arcsinh(Y_{lt}) = \sum_k \beta_k\widehat{\Delta L_{klt}} + \gamma \mathbf{X}_{lt} + \varepsilon_{lt} 
$${#eq-secondstage}

Note that the industry groups I use are `consumer_services`, `goods_producing`, `logistics_util`, `business_services`, and `public_nonprofit`. For the purposes of the second stage regression, `public_nonprofit` $\notin k$ as it forms the baseline for the factor `industry_pool`.

$Y_{lt}$ is the expected median home value in a given MSA, $l$, in a given year $t$. This regression is using an inverse hyperbolic sine transformation[^1] as I am ultimately looking for an elasticity.\
$\beta_k$ are a vector of coefficients for each industry $k$, note that I am using `public_nonprofit` as my baseline, and therefore will not show up on the results table.\
$\widehat{\Delta L_{klt}}$ is the treated local labor change annually for each industry estimated in @eq-firststage.\
$\mathbf{X}_{lt}$:
I use $arcsinh$ total population `tot_pop`, $arcsin$ household income `inc`, percentage of population over 25 that has a college degree `college_pct`, and percentage of entire population over the age of 25 `over25_pct` as controls. 
...
modelsummary(regz,
             ...
             coef_map = c("industry_pool::business_services:loc_growth" = "Business Services × Local Growth",
                          ...
                          "industry_pool::goods_producing:loc_growth" = "Goods Producing × Local Growth",
                          "industry_pool::logistics_util:loc_growth" = "Logistics/Utilities × Local Growth",
                          "fit_industry_pool::business_services:loc_growth" = "Business Services × Local Growth",
                          "fit_industry_pool::consumer_services:loc_growth" = "Consumer Services × Local Growth",
                          "fit_industry_pool::goods_producing:loc_growth" = "Goods Producing × Local Growth",
                          "fit_industry_pool::logistics_util:loc_growth" = "Logistics/Utilities × Local Growth",
                          "college_pct" = "% College Degree",
                          "over25_pct" = "% Residents over 25",
                           "asinh(inc)" = "Median Income",
                          "asinh(tot_pop)" = "MSA Population")
...
These are telling (and hard fought) results.

First the F-test confirms a very strong instrument for each industry, meaning the national effects are well accounted and I would argue that this captures much of the endogenous effects of national employment shifts on the local housing market. 

We can say with some confidence that shifts in the local labor market that come about due to local shocks do have a positive effect on housing market.

One percent increases in each of the following pools are possibly causal to increases in average home value, with significance at $99\%$:

Business Services: $2.7\%$\
Consumer Services: $6.8\%$\
Goods Production: $3.1\%$\
Logistics and Utilities: $8.7\%$

For our controls, they stay fairly close to the fixed effects predictions in @eq-fereg, as such one percent increases in the following pools corellate to increases of home value with statistical significance.



## Conclusion

I think this is strong start to observing local shocks. It serves as a reminder that economics is primarily *local* meaning that while the national levers on the economy do play a large part, local levers can and do have a strong impact as well. A local city manager or town planner may look at these data and it may either motvate them, or give them pause, on how a new policy favoring one industry or another can have an impact. 
